...
module handshake_sync #(
    ...
    // ---- Destination domain ----
    logic ack_dst;  // Acknowledge flag
...
// 9. Async FIFO (异步FIFO)
// ============================================================================
// Purpose: A complete asynchronous FIFO for transferring data between two
//          independent clock domains. This is the gold standard for CDC data
//          transfer when throughput matters.
//
// Architecture:
//   - Dual-port RAM with independent read/write clocks.
//   - Write pointer (clk_wr domain) and read pointer (clk_rd domain).
//   - Pointers are Gray-coded and synchronized across domains for
//     full/empty flag generation.
//
// Full/Empty logic:
//   - Empty: write pointer (synced to rd domain) == read pointer
//   - Full:  read pointer (synced to wr domain) == write pointer, BUT
//            the top 2 Gray bits differ (indicating a full wrap-around)
//
// Gray code full detection:
//   - In Gray code, FULL is when the MSB and MSB-1 are inverted between
//     the write and read pointers, and all other bits are equal.
// ============================================================================
module async_fifo #(
    parameter DATA_WIDTH = 8,
    ...
    // ---- Write and Read pointers (1 extra bit for full/empty distinction) ----
    logic [ADDR_WIDTH:0] wr_ptr_bin, rd_ptr_bin;  // Binary pointers
    logic [ADDR_WIDTH:0] wr_ptr_gray, rd_ptr_gray; // Gray-coded pointers

    // ---- Synchronized pointers ----
    logic [ADDR_WIDTH:0] wr_ptr_gray_sync1, wr_ptr_gray_sync2; // wr->rd domain
    logic [ADDR_WIDTH:0] rd_ptr_gray_sync1, rd_ptr_gray_sync2; // rd->wr domain
    ...
    // Full flag: MSB and MSB-1 are inverted, rest are equal
    // In Gray code, this means the write pointer has wrapped around
    ...
    // Empty flag: write and read Gray pointers are identical
