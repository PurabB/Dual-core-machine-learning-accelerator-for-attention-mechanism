...
$display("##### Estimated normalization result #####");
  ...
  for (t=0; t<total_cycle; t=t+1) begin
     ...
     for (q=0; q<col; q=q+1) begin
       ...
       if (sum[t] != 0)
         norm_result[t][q] = result[t][q] / (sum[t] >> 7);
       else
         norm_result[t][q] = 0;
     end

     $display("norm @cycle%2d: %d %d %d %d %d %d %d %d",
       t, norm_result[t][0], norm_result[t][1], norm_result[t][2], norm_result[t][3],
          norm_result[t][4], norm_result[t][5], norm_result[t][6], norm_result[t][7]);
  end
...
$display("##### Normalization: accumulate phase #####");
  ...
  #0.5 clk = 1'b0;
  pmem_rd = 0; execute = 0; pmem_add = 0;
  #0.5 clk = 1'b1;
...
////////////// Step 2: Normalization - divide & writeback ///////////////////

$display("##### Normalization: divide and writeback phase #####");

  // Read each pmem row, divide by sum, write back
  for (q=0; q<total_cycle; q=q+1) begin
    #0.5 clk = 1'b0;
    pmem_rd = 1;
    pmem_wr = 1;
  ...
  #0.5 clk = 1'b0;
  pmem_rd = 0; pmem_wr = 0; load = 0; pmem_add = 0;
  #0.5 clk = 1'b1;
